...
class Header3 extends Component {
	...
	static handleFlagClick() {
		if (getLanguage().toString() === 'nl') {
			Header3.selectPreferredLanguage('en');
		} else if (getLanguage().toString() === 'en') {
			Header3.selectPreferredLanguage('nl')
		}
	}
		...
		return (
			<div ref={this.header} className="navigation-container">
				<div className="navigation-wrapper">
					...
					{/*<div className="logo-wrapper"><NavLink to="/"><img className="logo" src={logo} alt="logo"/></NavLink></div>*/}
					...
					<div className={this.state.menuOpen ? "menu-open navigation-menu top" : "navigation-menu"}>
						<div className={this.state.menuOpen ? "menu-open navigation-links":" navigation-links"} >
							<li onClick={this.closeMenu}><NavLink activeClassName='active-link' to="/work">{this.props.t("header.work")}</NavLink></li>
							<li onClick={this.closeMenu}><NavLink activeClassName='active-link' to="/services">{this.props.t("header.services")}</NavLink></li>
							<li onClick={this.closeMenu}><NavLink activeClassName='active-link' to="/team">{this.props.t("header.about")}</NavLink></li>
							<li onClick={this.closeMenu}><NavLink activeClassName='active-link' to="/careers">{this.props.t("header.careers")}</NavLink></li>
							<li onClick={this.closeMenu}><NavLink activeClassName='active-link' to="/contact">{this.props.t("header.contact")}</NavLink></li>
						</div>
						<span className="translate-wrapper">
                        {/*eslint-disable-next-line*/}
							<a className="translate-button rounded" onClick={Header3.handleFlagClick} role="button"
							   style={{padding: "0"}}>
						...
						<div className={this.state.menuOpen ? "menu-open menu-overlay middle" : "menu-overlay middle"}> </div>
						<div className={this.state.menuOpen ? "menu-open menu-overlay bottom" : "menu-overlay bottom"}> </div>
					</div>
...
export default translate(Header3);
